...
searchbar.addEventListener("input", async function(e){
    ...
    for( let x of data){
        ...
        if(findMarkerDistance(userMarker,poiMarker)){
            let pid = "'" + x.pid + "'";
            let visitString = `<a href="#" onclick="registervisit(` + pid +`)">Register your visit</a>`
  ...
  function findMarkerDistance(userMark, poiMark){
    ...
    if (distance <= 4.02){//should be 0.02(20 metres to kilometers), change to greater values for test purposes
        return true
    }
    else {return false}
  }
